...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # PI-distill / OPSD: include alt-priv tensors for distillation losses
        distill_mode = self.config.get("distill_mode", None)
        distill_alpha = data.meta_info.get("distill_alpha", self.config.get("distill_alpha", 1.0))
        distill_coef = self.config.get("distill_coef", 0.0)

        # pi_distill: student loss using without-PI inputs
        has_student_loss = (
            distill_mode == "pi_distill"
            and distill_alpha < 1.0
            and "input_ids_without_priv" in data.batch.keys()
        )
        if has_student_loss:
            select_keys.extend(["input_ids_without_priv", "attention_mask_without_priv", "position_ids_without_priv"])

        # OPSD: forward KL loss — teacher uses with-PI, student is the main forward pass (without-PI)
        has_opsd_loss = (
            distill_mode == "opsd"
            and distill_coef > 0
            and "input_ids_with_priv" in data.batch.keys()
        )
        if has_opsd_loss:
            select_keys.extend(["input_ids_with_priv", "attention_mask_with_priv", "position_ids_with_priv"])
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                # Total response tokens in this mini-batch (for token-mean normalization).
                # Without this, each micro-batch independently divides by its own token count,
                # giving uneven per-token weight across micro-batches.
                mini_batch_num_tokens = mini_batch.batch["response_mask"].sum().item()
                self.config.global_batch_info["batch_num_tokens"] = mini_batch_num_tokens
                self.config.global_batch_info["dp_size"] = 1
                ...
                for micro_batch in micro_batches:
                    ...
                    # No per-micro-batch scaling — agg_loss divides by mini_batch_num_tokens
                    ...
                    # --- PI-distill: student loss (without-PI forward pass) ---
                    if has_student_loss:
                        teacher_log_prob = log_prob.detach()  # (bsz, resp_len)

                        # Student forward pass with without-PI inputs
                        student_inputs = {
                            "input_ids": model_inputs["input_ids_without_priv"],
                            "attention_mask": model_inputs["attention_mask_without_priv"],
                            "position_ids": model_inputs["position_ids_without_priv"],
                            "responses": model_inputs["responses"],
                            "pad_token_id": pad_token_id,
                        }
                        student_outputs = self._forward_micro_batch(
                            student_inputs, temperature=temperature, calculate_entropy=False,
                        )
                        student_log_prob = student_outputs["log_probs"]  # (bsz, resp_len)

                        # IS ratio: pi_student / pi_teacher (per token)
                        log_ratio = student_log_prob - teacher_log_prob
                        ratio = torch.exp(log_ratio)

                        # Clipped PPO-style student loss
                        eps_clip = self.config.get("student_clip_ratio", 0.2)
                        clipped_ratio = torch.clamp(ratio, 1.0 - eps_clip, 1.0 + eps_clip)

                        surr1 = ratio * advantages
                        surr2 = clipped_ratio * advantages
                        student_pg_loss = -torch.min(surr1, surr2)

                        student_loss = agg_loss(
                            loss_mat=student_pg_loss,
                            loss_mask=response_mask,
                            loss_agg_mode=loss_agg_mode,
                        )

                        # Weighted combination: alpha * teacher + (1 - alpha) * student
                        policy_loss = distill_alpha * policy_loss + (1.0 - distill_alpha) * student_loss

                        # Metrics
                        with torch.no_grad():
                            clip_frac = ((ratio - 1.0).abs() > eps_clip).float()
                            clip_frac = (clip_frac * response_mask).sum() / response_mask.sum().clamp(min=1)
                            mean_ratio = (ratio * response_mask).sum() / response_mask.sum().clamp(min=1)

                        micro_batch_metrics["actor/student_loss"] = student_loss.detach().item()
                        micro_batch_metrics["actor/teacher_loss"] = pg_loss.detach().item()
                        micro_batch_metrics["actor/student_ratio_mean"] = mean_ratio.item()
                        micro_batch_metrics["actor/student_clip_frac"] = clip_frac.item()
                        micro_batch_metrics["actor/distill_alpha"] = distill_alpha

                    # --- OPSD: forward KL loss (with-PI teacher → without-PI student) ---
                    # In OPSD, the main forward pass used WITHOUT-PI (student generates).
                    # We do an extra forward pass with WITH-PI to get teacher logprobs,
                    # then compute forward KL: KL(teacher || student) per token.
                    if has_opsd_loss:
                        import torch.nn.functional as _F

                        student_log_prob_opsd = log_prob  # main forward = without-PI

                        # Teacher forward pass with with-PI inputs
                        teacher_inputs = {
                            "input_ids": model_inputs["input_ids_with_priv"],
                            "attention_mask": model_inputs["attention_mask_with_priv"],
                            "position_ids": model_inputs["position_ids_with_priv"],
                            "responses": model_inputs["responses"],
                            "pad_token_id": pad_token_id,
                        }
                        ...
                        teacher_log_prob_opsd = teacher_outputs["log_probs"].detach()  # (bsz, resp_len)

                        # Forward KL per token: teacher_lp - student_lp
                        # = log pi(a|with_PI) - log pi(a|without_PI)
                        kl_per_token = teacher_log_prob_opsd - student_log_prob_opsd
                        opsd_kl_loss = agg_loss(
                            loss_mat=kl_per_token,
                            loss_mask=response_mask,
                            loss_agg_mode=loss_agg_mode,
                        )

                        policy_loss = policy_loss + distill_coef * opsd_kl_loss

                        with torch.no_grad():
                            mean_kl = (kl_per_token * response_mask).sum() / response_mask.sum().clamp(min=1)
                        micro_batch_metrics["actor/opsd_kl_loss"] = opsd_kl_loss.detach().item()
                        micro_batch_metrics["actor/opsd_mean_kl"] = mean_kl.item()
                        micro_batch_metrics["actor/distill_coef"] = distill_coef
