...
class ServerAdapter(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        """Batch generate sequences by calling the vLLM HTTP server synchronously.

        This re-implements the retired SPMD-style sync generation by dispatching
        individual requests to the colocated vLLM server actor and waiting for
        all results.
        """
        ...
        # Lazy-init server handle (same logic as _execute_method)
        if self.server_handle is None and self.rollout_rank == 0:
            self.server_handle = ray.get_actor(
                f"vllm_server_{self.replica_rank}_{self.node_rank}"
            )

        if self.rollout_rank != 0:
            # Only TP-rank 0 talks to the server; others return a stub.
            return prompts
        ...
        # Submit generation requests to the vLLM server
        futures = []
        for i in range(batch_size):
            mask = attention_mask[i].bool()
            prompt_ids_i = input_ids[i][mask].tolist()
            futures.append(
        ...
        outputs = ray.get(futures)  # list[TokenOutput]

        # Build padded response tensor
        all_response_ids = []
        for output in outputs:
            resp_ids = list(output.token_ids)
            if len(resp_ids) < response_length:
                resp_ids = resp_ids + [pad_token_id] * (response_length - len(resp_ids))
            else:
                resp_ids = resp_ids[:response_length]
            all_response_ids.append(resp_ids)

        responses = torch.tensor(all_response_ids, dtype=input_ids.dtype, device=input_ids.device)

        # Build full sequence tensors (prompt + response)
        full_input_ids = torch.cat([input_ids, responses], dim=1)
        response_mask = (responses != pad_token_id).long().to(attention_mask.device)
