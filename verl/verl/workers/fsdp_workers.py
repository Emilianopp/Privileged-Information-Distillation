...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def trainer_mode(self):
        """Context switch hybridengine back to trainer mode after rollout.

        Minimal implementation: release vLLM resources and clean up GPU cache.
        FSDP weights are reloaded on-demand by training methods when param_offload is enabled.
        """
        if self.config.rollout.free_cache_engine:
